...
export default function RaffleDetailModal({ raffleId, open, onClose }: RaffleDetailModalProps) {
    ...
    return (
        <Modal open={open} onClose={onClose} size="xl" title={detail?.title || 'Detalle de rifa'}
            lockBodyScroll closeOnBackdropClick={false}>
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex-1">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-4">
                            <div className="text-selected font-semibold">Precio: {detail?.price.toFixed(2)}
                                {detail?.currency}</div>
                            <div className="text-text-secondary text-sm">Seleccionados:
                                <span className="text-text-primary font-semibold">{selected.length}
                                </span>
                            </div>
                            <div className="text-text-secondary text-sm">Total:
                                <span className="text-text-primary font-semibold">{((detail?.price || 0) * selected.length).toFixed(2)}
                                    {detail?.currency}
                                </span>
                            </div>
                        </div>
                                        ...
                                        {selected.length === 0 ? (
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {selected.sort((a, b) => a - b).map(num => (
                                                    <button key={num}
                                                        onClick={() => setSelected(prev => prev.filter(x => x !== num))}
                ...
                <div className="border border-border-light rounded-xl p-3 bg-bg-primary">
                    ...
                    <div className="pb-4 md:pb-0">
                        <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 gap-2 place-items-center">
                            {pageNumbers.map(n => (
                                <NumberBadge key={n.number} n={n} />
                            ))}
                        </div>
                    </div>
                </div>

                <div className="flex flex-col md:flex-row justify-end gap-2 md:sticky md:bottom-0 md:bg-bg-primary md:pt-3 md:border-t md:border-border-light">
                    <Button variant="secondary" onClick={onClose}>Cerrar</Button>
                    <Button disabled={!selected.length}>Comprar ({selected.length})</Button>
                </div>
            </div>
        </Modal>
    );
